"use client";

import Link from "next/link";
import { motion } from "framer-motion";

export default function Footer() {
  const currentYear = new Date().getFullYear();

  return (
    <footer className="relative py-12 px-4 sm:px-6 lg:px-8 border-t border-border">
      <div className="max-w-7xl mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8 mb-8">
          {/* Brand */}
          <div className="md:col-span-2">
            <Link href="/" className="flex items-center gap-2 mb-4">
              <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-primary to-secondary flex items-center justify-center">
                <span className="text-background font-bold text-sm">DT</span>
              </div>
              <span className="font-display font-bold text-xl">
                Daily Tech <span className="gradient-text">News</span>
              </span>
            </Link>
            <p className="text-textMuted text-sm max-w-md">
              AI-powered daily technology news aggregation. Stay informed with
              curated insights from the world of tech, delivered every day at 10 AM.
            </p>
          </div>

          {/* Quick Links */}
          <div>
            <h4 className="font-display font-bold mb-4">Quick Links</h4>
            <ul className="space-y-2 text-sm">
              <li>
                <Link href="#news" className="text-textMuted hover:text-text transition-colors">
                  Latest News
                </Link>
              </li>
              <li>
                <Link href="#insights" className="text-textMuted hover:text-text transition-colors">
                  Insights
                </Link>
              </li>
              <li>
                <Link href="#about" className="text-textMuted hover:text-text transition-colors">
                  About
                </Link>
              </li>
            </ul>
          </div>

          {/* Connect */}
          <div>
            <h4 className="font-display font-bold mb-4">Connect</h4>
            <div className="flex gap-3">
              <a
                href="https://github.com/Xenonesis/Daily-tech-news"
                target="_blank"
                rel="noopener noreferrer"
                className="p-2 rounded-lg bg-surfaceHighlight border border-border hover:border-primary/50 transition-all duration-300"
              >
                <svg className="w-5 h-5 text-textMuted hover:text-text" fill="currentColor" viewBox="0 0 24 24">
                  <path
                    fillRule="evenodd"
                    d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-2.028 1.029-2.716-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0112 6.844c.85.004 1.705.115 2.504.337 1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.688 1.028 1.623 1.028 2.716 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.019 10.019 0 0022 12.017C22 6.484 17.522 2 12 2z"
                    clipRule="evenodd"
                  />
                </svg>
              </a>
            </div>
          </div>
        </div>

        <motion.div
          initial={{ opacity: 0 }}
          whileInView={{ opacity: 1 }}
          viewport={{ once: true }}
          className="pt-8 border-t border-border flex flex-col sm:flex-row items-center justify-between gap-4"
        >
          <p className="text-textMuted text-sm">
            © {currentYear} Daily Tech News. All rights reserved.
          </p>
          <p className="text-textMuted text-xs font-mono">
            Updated daily at 10:00 AM
          </p>
        </motion.div>
      </div>
    </footer>
  );
}
